...
def compare_with_database(root_window):
    ...
    def printInput():
        ...
        if text == "":
            ...
            string_1 = (
                "My top 3 predictions given the top 75 words are: "
                + "\n"
            ...
            string_2 = (
                "My top 3 predictions given the top 45 words are: "
                + "\n"
    ...
    # TextBox Creation
    inputtxt = tk.Text(frame1, height=50, width=150)

    inputtxt.place(relx=0.05, rely=0.20, relheight=0.4, relwidth=0.9)

    # Button Creation
    printButton = tk.Button(frame1, text="Predict", command=printInput)
    printButton.place(relx=0.375, rely=0.7, relheight=0.1, relwidth=0.15)

    skipPromptButton = tk.Button(frame1, text="Skip Prompt", command=skipPrompt)
    skipPromptButton.place(relx=0.375, rely=0.825, relheight=0.1, relwidth=0.15)
...
def edit_database(root_window):
    ...
    def display_file_content():
        ...
        selected_file_path = os.path.join(
            base_directory, selected_folder, selected_file
        )
    ...
    def add_new_file():
        ...
        file_path = filedialog.askopenfilename(
            parent=db, title="Select a Text File", filetypes=[("Text files", "*.txt")]
        )
        ...
        if file_path:
            ...
            try:
                ...
                tk.messagebox.showinfo(
                    "File Exists",
                    "File with that name already exists in the selected directory.",
                )
    ...
    def delete_selected_file():
        ...
        confirmation = tk.messagebox.askyesno(
            "Delete File", f"Do you want to delete the file '{selected_file}'?"
        )
        if confirmation:
            selected_folder = listbox.get(tk.ACTIVE)
    ...
    delete_folder_button = tk.Button(
        db, text="Delete Fingerprint", command=delete_selected_file
    )
    ...
    view_contents_button = tk.Button(
        db, text="View Contents", command=display_folder_contents
    )
    ...
    display_text_files_button = tk.Button(
        db, text="Display Prints", command=display_file_content
    )
...
def compare_two_prints(root_window):
    ...
    def submit():
        ...
        if results_1[0] == results_2[0]:
            results = (
                "The two prints are likely by the same author."
                + "\n\n"
                + "Author_1 Candidates: "
                + str(results_1)
                + "\n"
            ...
            results = (
                "The two prints are probably by the same author."
                + "\n\n"
                + "Author_1 Candidates: "
                + str(results_1)
                + "\n"
            ...
            results = (
                "The two prints are possibly or possibly not by the same author."
                + "\n\n"
                + "Author_1 Candidates: "
                + str(results_1)
                + "\n"
            ...
            results = (
                "The two prints are probably not by the same author"
                + "\n\n"
                + "Author_1 Candidates: "
                + str(results_1)
                + "\n"
            ...
            results = (
                "The two prints are unlikely to be by the same author"
                + "\n\n"
                + "Author_1 Candidates: "
                + str(results_1)
                + "\n"
...
def main():
    ...
    label.place(relx=0.5, rely=0.1, relheight=0.2, relwidth=0.8, anchor="center")
    # label.pack(padx= 100, pady= 100)
    # label = tk.Label(frame, text="CFSA Demo", font = ('Starzoom-Shavian', 25), bg="lightblue", pady=10)

    # Subtitle Widget
    label = tk.Label(
        frame, text="Demo Version", font=CyberFontSub, bg="lightblue", pady=10
    )
    label.place(relx=0.5, rely=0.27, relheight=0.2, relwidth=0.8, anchor="center")
    # Button Creation

    db_compare_button = tk.Button(
        frame,
        text="Compare With Database",
        command=lambda: compare_with_database(frame),
        font=CyberFontButton,
        width=35,
        height=5,
    )
    db_compare_button.place(
        relx=0.65, rely=0.4, relheight=0.1, relwidth=0.25, anchor="center"
    )
    # db_compare_button.pack()

    # button light up
    db_compare_button.bind(
        "<Enter>", lambda event, btn=db_compare_button: on_enter(btn)
    )
    db_compare_button.bind(
        "<Leave>", lambda event, btn=db_compare_button: on_leave(btn)
    )

    compare_two_prints_button = tk.Button(
        frame,
        text="Compare Two Prints",
        command=lambda: compare_two_prints(frame),
        font=CyberFontButton,
        width=35,
        height=5,
    )
    compare_two_prints_button.place(
        relx=0.35, rely=0.4, relheight=0.1, relwidth=0.25, anchor="center"
    )
    # compare_two_prints_button.pack()

    compare_two_prints_button.bind(
        "<Enter>", lambda event, btn=compare_two_prints_button: on_enter(btn)
    )
    compare_two_prints_button.bind(
        "<Leave>", lambda event, btn=compare_two_prints_button: on_leave(btn)
    )

    view_prints_button = tk.Button(
        frame,
        text="Placeholder",
        command=lambda: compare_two_prints(frame),
        font=CyberFontButton,
    ...
    view_prints_button.place(
        relx=0.65, rely=0.6, relheight=0.1, relwidth=0.25, anchor="center"
    )
    # compare_two_prints_button.pack()

    view_prints_button.bind(
        "<Enter>", lambda event, btn=view_prints_button: on_enter(btn)
    )
    view_prints_button.bind(
        "<Leave>", lambda event, btn=view_prints_button: on_leave(btn)
    )

    placeholder_button = tk.Button(
        frame,
        text="View Fingerprints",
        command=lambda: edit_database(frame),
        font=CyberFontButton,
        width=35,
        height=5,
    )
    placeholder_button.place(
        relx=0.35, rely=0.6, relheight=0.1, relwidth=0.25, anchor="center"
    )
    # compare_two_prints_button.pack()

    placeholder_button.bind(
        "<Enter>", lambda event, btn=placeholder_button: on_enter(btn)
    )
    placeholder_button.bind(
        "<Leave>", lambda event, btn=placeholder_button: on_leave(btn)
    )
    ...
    quit_button.place(relx=0.5, rely=0.9, relheight=0.1, relwidth=0.2, anchor="center")
