...
def predict(db,string,authors,N):
    ...
    for author in authors:
        ...
        if delta < min_delta:
            ...
            min_author = author
        # print(author,delta)
    predictions = {k: v for k, v in sorted(predictions.items(), key=lambda item: item[1])}
    return min_author,predictions,features

def measure(db,di,authors,N):
    ret = pd.DataFrame(columns = ['actual_author', 'work', 'prediction_1', 'delta_1', 'prediction_2', 'delta_2', 'prediction_3', 'delta_3', 'correct', 'in_top_3'])
    test_files = get_all_files(di)
    for file in test_files:
        parts = file.split("_-_")
        author = parts[0]
        name = parts[1].replace('.txt',"")
        min_author,predictions,features = predict(db,get_string_from_filename(di, author, name),authors,N)
        predictions = {k: v for k, v in sorted(predictions.items(), key=lambda item: item[1])}
        predictions_list = list(predictions.keys())
        # print(predictions)
        ret.loc[len(ret.index)] = [author, name, predictions_list[0], predictions[predictions_list[0]], predictions_list[1], predictions[predictions_list[1]], 
                                   predictions_list[2], predictions[predictions_list[2]], predictions_list[0]==author, author in predictions_list[:3]]
    return ret
...
def generate_corpus_dict(utterances):
    ...
    for utt in utterances:
        spkr = utt.speaker.id
        if utt.text == '[deleted]' or utt.text == '[removed]' or utt.text == '' or utt.speaker.id == '[deleted]':
            continue
...
def predict_formatted(prints, text, authors, N):
    ...
    string = "My top three guesses are:\n" + "Candidates: " + str(preds[:3]) + "\n" + "Deltas: " + str(deltas[:3]) +"\n"
    return string
